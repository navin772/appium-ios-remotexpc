...
export class LockdownService extends BasePlistService {
  ...
  async tryUpgradeToTLS(): Promise<void> {
    ...
    if (
      ...
      console.warn(
        'Missing required certificates or session info for TLS upgrade',
      );
      return;
    }
    ...
    try {
      ...
      if (!sessionResponse.enableSessionSSL) {
        console.log(
          'Device did not request TLS upgrade. Continuing with unencrypted connection.',
        );
        return;
      }
    } catch (error) {
  ...
  public async sendAndReceive(
    ...
    if (this._isUsingTLS && this._plistServiceAfterTLS) {
      ...
      try {
        const response = await this._plistServiceAfterTLS.sendPlistAndReceive(
          message,
          timeout,
        );
        return response;
...
export async function createLockdownServiceByUDID(
  ...
  try {
    ...
    if (devices.length === 0) {
      throw new Error(
        'No devices found. Make sure your device is connected and has been trusted on this computer.',
      );
    }
    ...
    for (const d of devices) {
      console.log(
        `- DeviceID: ${d.DeviceID}, Type: ${d.Properties.ConnectionType}, SerialNumber: ${d.Properties.SerialNumber}`,
      );
    }

    const device = devices.find(
      (device) => device.Properties.SerialNumber === udid,
    );
    if (!device) {
      console.error(
        `Device with UDID ${udid} not found in the list of connected devices`,
      );
      console.error(
        'Available UDIDs: ' +
      ...
      throw new Error(
        `Device with UDID ${udid} not found. Please check the UDID and ensure the device is connected.`,
      );
    }
