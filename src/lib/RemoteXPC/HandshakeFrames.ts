...
export class SettingsFrame extends Frame {
    ...
    parseBody(data: Buffer): void {
        if (this.flags.has('ACK') && data.length > 0) {
            throw new InvalidDataError(`SETTINGS ack frame must not have payload: got ${data.length} bytes`);
        }
...
export class WindowUpdateFrame extends Frame {
    ...
    parseBody(data: Buffer): void {
        if (data.length > 4) {
            throw new InvalidFrameError(`WINDOW_UPDATE frame must have 4 byte length: got ${data.length}`);
        }
        ...
        if (!(this.windowIncrement >= 1 && this.windowIncrement <= 0x7FFFFFFF)) {
            throw new InvalidDataError('WINDOW_UPDATE increment must be between 1 to 2^31-1');
        }
