import { XpcConstants, Http2Constants } from './Constants.js';
import { DataFrame, HeadersFrame, SettingsFrame, WindowUpdateFrame } from './HandshakeFrames.js';
import { encodeMessage , type XPCMessage} from './XPCProtocol.js';
...
class Handshake {
    ...
    async perform(): Promise<void> {
        ...
        const windowUpdateFrame: WindowUpdateFrame = new WindowUpdateFrame(0, 983041);
        await this.sendFrame(windowUpdateFrame.serialize());

        // Step 4: Send a HEADERS frame on stream 1.
        const headersFrameRoot: HeadersFrame = new HeadersFrame(Http2Constants.ROOT_CHANNEL, Buffer.from(''), ['END_HEADERS']);
        ...
        const headersFrameReply: HeadersFrame = new HeadersFrame(Http2Constants.REPLY_CHANNEL, Buffer.from(''), ['END_HEADERS']);
