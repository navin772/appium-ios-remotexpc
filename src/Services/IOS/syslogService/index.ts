...
class SyslogService {
     ...
     */
    async start(Service: Service, pid: number = -1, tunnelManager: TunnelManager): Promise<void> {
        ...
        tunnelManager.on('data', (packet: Packet) => {
            if (packet.protocol === 'TCP') {
                ...
                if (this.isMostlyPrintable(packet.payload)) {
                    ...
                    console.log('  Message:', packet.payload.toString().replace(/[^\x20-\x7E]/g, ''));
                } else {
     ...
     * @param service
     */
    private async startLockdownWithoutCheckin(service: Service): Promise<ServiceConnection> {
     ...
     * @param service
     */
    private async startLockdownService(service: Service): Promise<ServiceConnection> {
